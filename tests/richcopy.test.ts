 ...
 * AgentPanel.tsx): the MathJax-rendered formulas leave the selection as their LaTeX source —
...
const INLINE = '<div class="agent-msg">energy <span class="agent-math" data-latex="E=mc^{2}"><mjx-container class="MathJax">𝐸=𝑚𝑐²</mjx-container></span> matters</div>';
...
describe('latexSelectionText', () => {
  ...
  it('display math copies with \\[ … \\] on their own lines', () => {
    const root = setup('<div class="agent-msg">so<div class="agent-math-block"><span class="agent-math display" data-latex="\\int f\\,dx" data-display="1"><mjx-container class="MathJax">∫f dx</mjx-container></span></div>holds</div>');
    expect(latexSelectionText(mkSel(over(root)))).toBe('so\n\\[\n\\int f\\,dx\n\\]\nholds');
  });
  it('a drag inside one formula’s own MathJax DOM takes the formula whole', () => {
    const glyphs = setup(INLINE).querySelector('mjx-container')!.firstChild as Text;
