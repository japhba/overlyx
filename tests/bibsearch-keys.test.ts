...
bs.setFetch((async (input: any, init?: any) => {
  ...
  if (url.startsWith('https://serpapi.com/search.json?engine=google_scholar&')) return json({ organic_results: [
    { result_id: 'abc123', title: 'Chaos in random neural networks', link: 'https://journals.aps.org/prl/abstract/10.1103/PhysRevLett.61.259', publication_info: { summary: 'H Sompolinsky, A Crisanti, HJ Sommers - Physical review letters, 1988 - APS', authors: [{ name: 'H Sompolinsky' }, { name: 'A Crisanti' }] }, inline_links: { cited_by: { total: 1600 } } },
...
describe('Scholar-grade backends', () => {
  ...
  it('with a SerpApi key Google Scholar leads: its order is kept, the open indexes only add DOIs and citation counts', async () => {
    const hits = await bs.searchLiterature('chaos in random neural networks');
    ...
    expect(hits[0].authors).toEqual(['H Sompolinsky', 'A Crisanti']);
