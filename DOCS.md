...
* **Fonts, for the editor and for the PDF separately** (like LyX's screen fonts and document fonts;
  ...
  (`prefs.editorMathFont`): *Matching the text font* (the math font drawn for the face, the default),
  KaTeX's own Computer Modern, or one of 30 OpenType math fonts (Latin Modern, New Computer Modern,
  STIX Two, XITS, Libertinus, the TeX Gyre fonts, Asana, Euler, Garamond, XCharter, Erewhon, KpMath,
  Concrete, Old Standard, GFS Neohellenic, IBM Plex, PL46, Fira, Lete Sans, Noto Sans, Arsenal,
  Luciole, Pennstander Math; *Fira Math, letters from the text font* is the sans face's match, since
  San Francisco cannot be served). The commercial ones (Cambria, Lucida, Minion Math) and those not
  on CTAN are left out. The sample under the choices is that question's test document plus a line of
  accents. All fonts are served with the client (`client/src/fonts/web`, generated by
  `scripts/build-editor-fonts.py` from TeX Live, CTAN and google/fonts — run it again after changing
  its tables; needs `pip install fonttools brotli`), declared in `fonts/web/webfonts.css` and fetched
  by the browser only once text uses them; the CSPs allow no other font source. KaTeX cannot read an
  OpenType MATH table, so the script takes each math font apart into faces that stand in for KaTeX's
  fonts glyph by glyph (main symbols and accents, math italic / bold / bold italic, script, fraktur,
  double-struck, sans, typewriter, Size1–4 for big operators and delimiters: the font's size variant
  closest to KaTeX's glyph, fitted to its box), all scaled to Computer Modern's x-height, so KaTeX's
  metrics fit; `fonts/editorfont.ts` sets `--mf-<face>` and `data-math-font` on html, and styles.css
  puts each face before the KaTeX font it replaces. KaTeX keeps its layout: spacing, rules, the SVG
  radical sign, `\vec` and the wide accents, delimiters taller than `\Bigg`. Formulas are sized to
  the text face's x-height (`mathScale`, 1.1 times it, as KaTeX beside CMU Serif always was); `\text`
  in formulas is the text face. The glyphs KaTeX gets wrong otherwise are drawn to fit: accents
  centred on KaTeX's, `\not` (so ≠ is centred on the font's own =), primes fitted to KaTeX's
  superscript prime. `scratch/mathfont-probe.mjs <ids>` renders the sample in each math font. *Document ▸ Settings
