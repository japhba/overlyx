...
export function classOptions(ctx: ExportContext): string {
  const { bp, dc } = ctx;
  const opts: string[] = [];
...
export function amsPackages(ctx: ExportContext): string {
...
export function colorOptions(ctx: ExportContext): string {
  const f = ctx.features;
  const { bp } = ctx;
  let s = '';
...
export function packages(ctx: ExportContext): string {
  const f = ctx.features;
  const { bp } = ctx;
  let s = '';
...
export function hyperrefPreamble(ctx: ExportContext): string {
  const { pdf } = ctx.bp;
  const f = ctx.features;
  let opt = '';
  let hyperset = '';
...
export function lyxMacros(ctx: ExportContext): string {
  const f = ctx.features;
  let m = '';
...
export function tclassPreamble(ctx: ExportContext): string {
...
export function tclassI18nPreamble(ctx: ExportContext): string {
  const { dc, features } = ctx;
...
export function babelCall(ctx: ExportContext): string {
...
export function writePreamble(ctx: ExportContext, popts: { noMacros?: boolean } = {}): string {
  ...
  // \makeatletter block
  let at = '';
  const macros = popts.noMacros ? '' : lyxMacros(ctx);
  if (macros.trim()) at += '\n%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% LyX specific LaTeX commands.\n' + macros + '\n';
  const tc = popts.noMacros ? '' : tclassPreamble(ctx);
