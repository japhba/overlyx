...
class BodyParser {
  ...
  private handleCommand(s: Scanner, ctx: TextCtx, st: State, t: Tok, stop: Stop): StopReason | null {
    ...
    // the class's title command (\maketitle & co) is regenerated by the writer
    if (name === this.dc.titleLatexName && this.dc.titleLatexType === 'CommandAfter') { if (s.peekChar() === '{' && s.peekChar(1) === '}') s.pos += 2; this.endPar(ctx); return null; }
    ...
    if (name === 'maketitle') { if (s.peekChar() === '{' && s.peekChar(1) === '}') s.pos += 2; this.endPar(ctx); return null; }
    ...
    const uni = this.unicodeRev.get('\\' + name);
