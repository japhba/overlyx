...
export const config = {
   ...
   * key. Both defaults are fast non-thinking models: a ⌘K rewrite is interactive, and Gemini 3.7
   * Flash's mandatory reasoning made it take 7–16 s per rewrite (its hidden reasoning cannot be
   * switched off; re-measured 2026-09-02, scratch/ai-rewrite-bench.mjs) — 3.1 Flash Lite answers
   * in ~1 s and follows the document's notation. The whole paper still goes along as context
   * (a million tokens). Autocomplete: 2.5 Flash Lite wrote proper sentences in 0.5–0.9 s where
   * thinking models answered with a word or nothing (2026-08-29, scratch/ai-bench.mjs).
   */
  ai: {
    model: (process.env.OVERLYX_AI_MODEL ?? 'google/gemini-3.1-flash-lite').trim(),
